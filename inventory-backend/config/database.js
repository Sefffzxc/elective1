...
const r = rethinkdbdash({
  servers: [
    { host: process.env.RETHINKDB_HOST_A || '192.168.1.15', port: parseInt(process.env.RETHINKDB_PORT) || 28015 },
    { host: process.env.RETHINKDB_HOST_B || '192.168.1.15', port: parseInt(process.env.RETHINKDB_PORT) || 28015 }
  ],
...
export const createDatabase = async () => {
  try {
    ...
    if (!tableList.includes('inventory_logs')) {
      await r.db('ordering_system').tableCreate('inventory_logs').run();
